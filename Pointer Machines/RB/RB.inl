...
bool RB<K,D>::insert(const K & _key, const D & _data){
    insert(_key,_data,&p_root,NULL);
    return true;
}

template <class K, class D>
Node<K,D> * RB<K,D>::insert(const K & _key, const D & _data, Node<K,D> ** n,  Node<K,D> ** p){
    if(!(*n)){
...
template <class K, class D>
bool RB<K,D>::find_node(const K & _key, Node<K,D> *temp){
    temp = p_root;
    while(temp)
    {
        if( (temp)->key == _key ) return true;
...
template <class K, class D>
void RB<K,D>::turn_side( Node<K,D> ** n, bool side){
...
template <class K, class D>
void RB<K,D>::inorder(Node<K,D> *n){
    if(!n) return;
...
template <class K, class D>
void RB<K,D>::update_color(Node<K,D> * n){
    if(!n) return;
    p_root->color =BLACK;
}

template <class K, class D>
bool RB<K,D>::is_red(Node<K, D> * n){
    if(!n) return false;
...
template <class K, class D>
bool RB<K,D>::is_black(Node<K, D> * n){
    if(!n) return true;
...
template <class K, class D>
bool RB<K,D>::successor(Node<K,D> *nodo, Node<K,D> *sucesor){
...
template<class K, class D>
bool RB<K,D>::draw(Node<K,D> *n, ofstream & os){
    if(!n) return false;
    queue<Node<K,D> *> q;
